...
async def get_stages(
    ...
    query = select(Stage).where(Stage.event_id == active_event.id).order_by(Stage.order)
...
async def get_current_stage(db: AsyncSession = Depends(get_session)):
    ...
    result = await db.execute(
        select(Stage).where(Stage.is_active == True, Stage.event_id == active_event.id)
    )
    current_stage = result.scalar_one_or_none()

    if not current_stage:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No active stage found"
        )
...
async def get_available_transitions(
    ...
    all_stages_result = await db.execute(
        select(Stage).where(Stage.event_id == active_event.id).order_by(Stage.order)
    )
    ...
    return {"current_stage": current_stage, "available_transitions": available_stages}
...
async def activate_stage(
    ...
    if not await check_admin_role(current_user, db):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Only admin can change stages"
        )

    new_stage_result = await db.execute(select(Stage).where(Stage.id == stage_id))
...
async def create_stage(
    ...
    if new_stage.is_auto_activate and new_stage.auto_activate_at:
        ...
        logging.info(
            f"Создан этап с автоматической активацией: {new_stage.name}, время: {new_stage.auto_activate_at}"
        )
...
async def update_stage(
    ...
    if not stage:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Этап не найден"
        )
    ...
    provided_fields = stage_data.model_fields_set
    ...
    if "is_auto_activate" in provided_fields:
        stage.is_auto_activate = stage_data.is_auto_activate
    if "auto_activate_at" in provided_fields:
